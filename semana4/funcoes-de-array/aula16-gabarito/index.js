let arrDespesas = [] // guarda array com listas renderizadas na div despesa
imprimirDespesas(arrDespesas) // recebe como parâmetro a variável acima
imprimirExtrato()
...
function imprimirExtrato(){
    ...
    arrDespesas.forEach((extrato) => {
        if (despesa.tipo === 'alimentação') {
            gastoAlimentação += despesa.valor
        } else if (despesa.tipo === 'utilidades') {
            gastoUtilidades += despesa.valor
        } else {
            gasto += despesa.valor
    }
    gastoTotal += gastoAlimentação + gastoUtilidades + gastoViagem
    })
